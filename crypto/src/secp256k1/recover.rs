...
/// Recovery with detailed logging for debugging RISC-V issues.
#[cfg(feature = "secp256k1-static-context")]
pub fn recover_with_logging(
    message: &crate::k256::Scalar,
    signature: &crate::k256::ecdsa::Signature,
    recovery_id: &crate::k256::ecdsa::RecoveryId,
    tx_num: usize,
    ...
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" starting recovery");
    uart_log::newline();

    let (mut sigr, mut sigs) = Scalar::from_signature(signature);
    let message_scalar = Scalar::from_k256_scalar(*message);

    // Log sigr and sigs
    let sigr_bytes = sigr.to_repr();
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" sigr=");
    uart_log::write_hex_slice(&sigr_bytes);
    uart_log::newline();

    let sigs_bytes = sigs.to_repr();
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" sigs=");
    uart_log::write_hex_slice(&sigs_bytes);
    uart_log::newline();

    // We go through bytes because it's mod GROUP_ORDER and later we need mod BASE FIELD
    let mut brx = sigr.to_repr();

    // Log brx immediately after to_repr()
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" brx after to_repr()=");
    uart_log::write_hex_slice(&brx);
    uart_log::newline();

    if recovery_id.is_x_reduced() {
        uart_log::write_str("[recover] TX#");
        uart_log::write_usize(tx_num);
        uart_log::write_str(" x_reduced=true, adjusting brx");
        uart_log::newline();
            ...
            .into_option()
        {
            ...
            None => {
                uart_log::write_str("[recover] TX#");
                uart_log::write_usize(tx_num);
                uart_log::write_str(" ERROR: overflow in x_reduced");
                uart_log::newline();
                return Err(Secp256k1Err::OperationOverflow);
            }
        }
    }
    ...
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" is_y_odd=");
    uart_log::write_usize(is_odd as usize);
    uart_log::newline();

    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" decompressing point from brx using decompress_to");
    uart_log::newline();

    // Use decompress_to to avoid return value corruption on RISC-V
    let mut x = Affine::DEFAULT;
    if !Affine::decompress_to_with_logging(&brx, is_odd, &mut x, tx_num) {
        uart_log::write_str("[recover] TX#");
        uart_log::write_usize(tx_num);
        uart_log::write_str(" ERROR: decompress_to failed");
        uart_log::newline();
        return Err(Secp256k1Err::InvalidParams);
    }

    // Log the decompressed point x and y
    let x_bytes = x.x.to_bytes();
    let y_bytes = x.y.to_bytes();
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" decomp_x=");
    uart_log::write_hex_slice(&x_bytes);
    uart_log::newline();

    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" decomp_y=");
    ...
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" calling ecmult");
    uart_log::newline();

    let mut pk = ecmult(&xj, &sigs, &sigr, &ECRECOVER_CONTEXT).to_affine();
    pk.normalize_in_place();
    ...
    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" pk.x=");
    uart_log::write_hex_slice(&pk_x_bytes);
    uart_log::newline();

    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" pk.y=");
    uart_log::write_hex_slice(&pk_y_bytes);
    uart_log::newline();

    uart_log::write_str("[recover] TX#");
    uart_log::write_usize(tx_num);
    uart_log::write_str(" pk.infinity=");
    uart_log::write_usize(pk.is_infinity() as usize);
    uart_log::newline();

    if pk.is_infinity() {
        uart_log::write_str("[recover] TX#");
        uart_log::write_usize(tx_num);
        uart_log::write_str(" ERROR: recovered infinity");
        uart_log::newline();
        return Err(Secp256k1Err::RecoveredInfinity);
    }
