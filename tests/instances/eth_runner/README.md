...
Note: `eth_runner` is excluded from the main workspace, so commands must be run from this directory (`tests/instances/eth_runner`).

### Single run

From this directory (`tests/instances/eth_runner`), run:

```shell
RUST_LOG=eth_runner=info cargo run --release --features rig/no_print,rig/unlimited_native -- single-run --block-dir blocks/22244135 --randomized
```
...
This command will fetch blocks in the range [19299000, 19299005] from the Ethereum archive node `YOUR_RPC_ENDPOINT`. It creates a local database to cache some RPC information.
