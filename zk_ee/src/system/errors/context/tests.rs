...
mod tests {
    ...
    fn test_lazy_detailed_evaluation() {
        ...
        #[cfg(all(not(any(target_arch = "riscv32", target_arch = "riscv64")), feature = "detailed_errors"))]
        assert_eq!(DETAILED_CALL_COUNT.load(Ordering::Relaxed), 1);

        #[cfg(not(all(not(any(target_arch = "riscv32", target_arch = "riscv64")), feature = "detailed_errors")))]
